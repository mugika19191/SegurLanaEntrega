<?php 
	...
	function createUser($conn,$name,$name2,$tel,$email,$nan,$data,$pass){
		...
		mysqli_stmt_bind_param($stmt,"ssissss",$name,$name2,$tel,$email,$nan,$data,$hashed_password);
		mysqli_stmt_execute($stmt);
		mysqli_stmt_close($stmt);


		$timestamp = time();
		$sql="INSERT INTO intento (nan,wrong,denbora) VALUES (?,0,?);";
		$stmt=mysqli_stmt_init($conn);
		if(!mysqli_stmt_prepare($stmt,$sql)){
			header("location: ../registro.php?error=stmtfailed");
			exit();
		}
		mysqli_stmt_bind_param($stmt,"si",$nan,$timestamp);
		mysqli_stmt_execute($stmt);
		mysqli_stmt_close($stmt);
	...
	function loginUser($conn,$user,$pass){
		...
		if(!password_verify($pass, $userpass)){		
			addTry($conn,$user);
			echo"<script>window.location.href = '../inicioSesion.php?error=wronglogin2';</script>";
			exit();
		}
		else {
			session_start();
	...
	function addTry($conn,$user){
		if (checkWrongs($conn,$user)>=5){
			if((time()-checkTimeRemain($conn,$user))>=300){
				resetCounter($conn,$user);
			}
			else{
				echo '<script>alert("5 minutu igaron behar dira berriro saiatzeko.")</script>';
			}
			return;
		}
		$sql="UPDATE intento SET wrong = wrong + 1, denbora = ?  WHERE nan = ?;";
		
		//$sql="SELECT e.* FROM edukia e INNER JOIN kontratazioa k on k.erabiltzaileNan=e.nan WHERE k.erabiltzaileNan=?;";
		$stmt=mysqli_stmt_init($conn);
		if(!mysqli_stmt_prepare($stmt,$sql)){
			header("location: ../inicioSesion.php?error=stmtfailed");
			exit();
		}
		
		mysqli_stmt_bind_param($stmt,"is",time(),$user);
		mysqli_stmt_execute($stmt);
		mysqli_stmt_close($stmt);
		
	}
	function checkWrongs($conn,$nan){
		$sql="SELECT wrong FROM intento WHERE nan = ?;";
		...
		mysqli_stmt_close($stmt);
		return intval($number["wrong"]);
	}
	function checkTimeRemain($conn,$nan){
		$sql="SELECT denbora FROM intento WHERE nan = ?;";
		...
		$number=mysqli_fetch_assoc($count);
		mysqli_stmt_close($stmt);
		return intval($number["denbora"]);
	}
	function resetCounter($conn,$user){
		$sql="UPDATE intento SET wrong = 1, denbora = ?  WHERE nan = ?;";
